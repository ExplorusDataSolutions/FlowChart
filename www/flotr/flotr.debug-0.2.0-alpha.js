...
var Flotr = {
	...
	getSeries : function(data) {
		return data.collect(function(serie) {
			var i;
			serie = (serie.data) ? Object.clone(serie) : {
				data : serie
			};
	...
	engineeringNotation : function(value, precision, base) {
		var sizes = ['Y', 'Z', 'E', 'P', 'T', 'G', 'M', 'k', ''], fractionSizes = ['y', 'z', 'a', 'f', 'p', 'n', '�', 'm', ''], total = sizes.length;
		base = base || 1000;
	...
	toPixel : function(val) {
		...
		//((val-Math.round(val) < 0.4) ? (Math.floor(val)-0.5) : val);
	},
...
Flotr.Graph = Class.create({
	...
	setOptions : function(opts) {
		...
		for( i = 0; colors.length < neededColors; ) {
			...
			var sign = variation % 2 == 1 ? -1 : 1, factor = 1 + sign * Math.ceil(variation / 2) * 0.2;
	...
	getTextDimensions : function(text, canvasStyle, HtmlStyle, className) {
		...
		if(!this.options.HtmlText && this.textEnabled) {
			...
			var dummyDiv = this.el.insert('<div style="position:absolute;top:-10000px;'+HtmlStyle+'" class="'+className+' flotr-dummy-div">' + text + '</div>').select(".flotr-dummy-div")[0], dim = dummyDiv.getDimensions();
			dummyDiv.remove();
			return dim;
		}
	},
	...
	loadDataGrid : function() {
		...
		for( i = 0; i < s.length; ++i) {
			s[i].data.each(function(v) {
				var x = v[0], y = v[1], r = dg.find(function(row) {
					return row[0] == x
				});
				if(r)
					r[i + 1] = y;
	...
	findDataRanges : function() {
		...
		if(s.length > 0) {
			...
			for( i = 0; i < s.length; ++i) { data = s[i].data, xaxis = s[i].xaxis, yaxis = s[i].yaxis;
	...
	calculateTicks : function(axis) {
		...
		if(o.ticks) {
			...
			if(o.mode == 'time') {
				var tu = Flotr.Date.timeUnits, spec = Flotr.Date.spec, delta = (axis.max - axis.min) / axis.options.noTicks, size, unit;
				...
				for( i = 0; start + i * axis.tickSize <= axis.max; ++i) {
					...
					if (decimals == 0 && Math.abs(axis.max - axis.min) < 1)	
						decimals = 1 - Math.floor(Math.log(axis.tickSize) / Math.LN10);
					if (decimals < 0)
						decimals = 0;
	...
	calculateSpacing : function() {
		...
		p.bottom += (options.grid.circular ? 0 : (x.options.showLabels ? (x.maxLabel.height + margin) : 0)) + (x.options.title ? (x.titleSize.height + margin) : 0) + maxOutset;

		p.top += (options.grid.circular ? 0 : (x2.options.showLabels ? (x2.maxLabel.height + margin) : 0)) + (x2.options.title ? (x2.titleSize.height + margin) : 0) + this.subtitleHeight + this.titleHeight + maxOutset;

		p.left += (options.grid.circular ? 0 : (y.options.showLabels ? (y.maxLabel.width + margin) : 0)) + (y.options.title ? (y.titleSize.width + margin) : 0) + maxOutset;

		p.right += (options.grid.circular ? 0 : (y2.options.showLabels ? (y2.maxLabel.width + margin) : 0)) + (y2.options.title ? (y2.titleSize.width + margin) : 0) + maxOutset;
	 ...
	 */
	drawGrid : function() {
		var v, o = this.options, ctx = this.ctx, a;
		...
		if(o.grid.circular) {
			...
			var radius = Math.min(this.plotHeight, this.plotWidth) * o.radar.radiusRatio / 2, sides = this.axes.x.ticks.length, coeff = 2 * (Math.PI / sides), angle = -Math.PI / 2;
			...
			if(o.grid.verticalLines) {
				...
				for(var i = 0; i < a.ticks.length; ++i) {
					...
					if((v <= a.min || v >= a.max) || (v == a.min || v == a.max) && o.grid.outlineWidth != 0)
						continue;
	...
	drawOutline : function() {
		...
		if(o.grid.circular) {
			...
			// Draw axis/grid border.
			var lw = o.grid.outlineWidth, orig = 0.5 - lw + ((lw + 1) % 2 / 2);
			ctx.lineWidth = lw;
			ctx.strokeStyle = o.grid.color;
			ctx.lineJoin = 'miter';
	...
	drawLabels : function() {
		...
		var noLabels = 0, axis, xBoxWidth, i, html, tick, left, top, options = this.options, ctx = this.ctx, a = this.axes;
		...
		if(options.grid.circular) {
			ctx.save();
			ctx.translate(this.plotOffset.left + this.plotWidth / 2, this.plotOffset.top + this.plotHeight / 2);
			var radius = this.plotHeight * options.radar.radiusRatio / 2 + options.fontSize, sides = this.axes.x.ticks.length, coeff = 2 * (Math.PI / sides), angle = -Math.PI / 2;
			...
			for( i = 0; i < axis.ticks.length && axis.options.showLabels; ++i) {
				...
				var x = Math.cos(i * coeff + angle) * radius, y = Math.sin(i * coeff + angle) * radius;
		...
		if(!options.HtmlText && this.textEnabled) {
			...
			for( i = 0; i < axis.ticks.length && axis.options.showLabels && axis.used; ++i) {
				...
				ctx.drawText(tick.label, this.plotOffset.left + left, this.plotOffset.top + this.plotHeight + options.grid.labelMargin, style);
			}
			...
			for( i = 0; i < axis.ticks.length && axis.options.showLabels && axis.used; ++i) {
				...
				ctx.drawText(tick.label, this.plotOffset.left + left, this.plotOffset.top + options.grid.labelMargin, style);
			}
			...
			for( i = 0; i < axis.ticks.length && axis.options.showLabels && axis.used; ++i) {
				...
				ctx.drawText(tick.label, this.plotOffset.left - options.grid.labelMargin, this.plotOffset.top + top, style);
			}
			...
			for( i = 0; i < axis.ticks.length && axis.options.showLabels && axis.used; ++i) {
				...
				ctx.drawText(tick.label, this.plotOffset.left + this.plotWidth + options.grid.labelMargin, this.plotOffset.top + top, style);
			...
			if(axis.options.showLabels) {
				for( i = 0; i < axis.ticks.length; ++i) {
					tick = axis.ticks[i];
					if(!tick.label || tick.label.length == 0 || (this.plotOffset.left + axis.d2p(tick.v) < 0) || (this.plotOffset.left + axis.d2p(tick.v) > this.canvasWidth))
						continue;
			...
			if(axis.options.showLabels) {
				for( i = 0; i < axis.ticks.length; ++i) {
					...
					if(!tick.label || tick.label.length == 0 || (this.plotOffset.top + axis.d2p(tick.v) < 0) || (this.plotOffset.top + axis.d2p(tick.v) > this.canvasHeight))
						continue;
					var top = this.plotOffset.top + axis.d2p(tick.v) - axis.maxLabel.height / 2;
					if (i == 0) top -= 18;
					html.push('<div style="position:absolute;top:' + (top) + 'px;left:0;width:auto;text-align:right;' + (axis.options.color ? ('color:' + axis.options.color + ';') : '') + '" class="flotr-grid-label">' + tick.label + '</div>');
				}
			}
	...
	insertLegend : function() {
		...
		var series = this.series, plotOffset = this.plotOffset, options = this.options, legend = options.legend, fragments = [], rowStarted = false, ctx = this.ctx, i;
		...
		if(noLegendItems) {
			if(!options.HtmlText && this.textEnabled && !$(legend.container)) {
				...
				var p = legend.position, m = legend.margin, lbw = legend.labelBoxWidth, lbh = legend.labelBoxHeight, lbm = legend.labelBoxMargin, offsetX = plotOffset.left + m, offsetY = plotOffset.top + m;
				...
				for( i = 0; i < series.length; ++i) {
					...
					var s = series[i], label = legend.labelFormatter(s.label), boxWidth = legend.labelBoxWidth, boxHeight = legend.labelBoxHeight, opacity = 'opacity:' + s.bars.fillOpacity + ';filter:alpha(opacity=' + s.bars.fillOpacity * 100 + ');', color = 'background-color:' + ((s.bars.show && s.bars.fillColor && s.bars.fill) ? s.bars.fillColor : s.color) + ';';
					...
					if(options.legend.container != null) {
						...
						if(options.legend.backgroundOpacity != 0.0) {
							...
							if(c == null) {
								...
								c = this.processColor(tmp, null, {
									opacity : 1
								});
							}
							this.el.insert('<div class="flotr-legend-bg" style="position:absolute;width:' + div.getWidth() + 'px;height:' + div.getHeight() + 'px;' + pos + 'background-color:' + c + ';"> </div>').select('div.flotr-legend-bg').first().setOpacity(options.legend.backgroundOpacity);
						}
					}
				}
			}
	...
	getEventPosition : function(event) {
		var offset = this.overlay.cumulativeOffset(), pointer = Event.pointer(event), rx = (pointer.x - offset.left - this.plotOffset.left), ry = (pointer.y - offset.top - this.plotOffset.top);
	...
	touchmoveHandler : function(event) {
		...
		var rx = (pointer.x - offset.left - this.plotOffset.left), ry = (pointer.y - offset.top - this.plotOffset.top);
	...
	fireSelectEvent : function() {
		var a = this.axes, s = this.selection, x1, x2, y1, y2;
	...
	clearSelection : function() {
		...
		var prevSelection = this.prevSelection, lw = this.octx.lineWidth, plotOffset = this.plotOffset, x = Math.min(prevSelection.first.x, prevSelection.second.x), y = Math.min(prevSelection.first.y, prevSelection.second.y), w = Math.abs(prevSelection.second.x - prevSelection.first.x), h = Math.abs(prevSelection.second.y - prevSelection.first.y);

		this.octx.clearRect(x + plotOffset.left - lw / 2 + 0.5, y + plotOffset.top - lw / 2 + 0.5, w + lw, h + lw);
	...
	setSelection : function(area, preventEvent) {
		var options = this.options, xa = this.axes.x, ya = this.axes.y, vertScale = ya.scale, hozScale = xa.scale, selX = options.selection.mode.indexOf('x') != -1, selY = options.selection.mode.indexOf('y') != -1;
	...
	drawSelection : function() {
		var prevSelection = this.prevSelection, s = this.selection, octx = this.octx, options = this.options, plotOffset = this.plotOffset;

		if(prevSelection != null && s.first.x == prevSelection.first.x && s.first.y == prevSelection.first.y && s.second.x == prevSelection.second.x && s.second.y == prevSelection.second.y)
			return;
	...
	drawCrosshair : function(pos) {
		var octx = this.octx, options = this.options, plotOffset = this.plotOffset, x = plotOffset.left + pos.relX + 0.5, y = plotOffset.top + pos.relY + 0.5;
	...
	clearHit : function() {
		...
		var prevHit = this.prevHit, plotOffset = this.plotOffset, s = prevHit.series, lw = s.bars.lineWidth, xa = prevHit.xaxis, ya = prevHit.yaxis;

		if(!s.bars.show) {
			var r = s.points.radius;
			this.octx.clearRect(xa.d2p(prevHit.x) + plotOffset.left - r * 2, ya.d2p(prevHit.y) + plotOffset.top - r * 2, r * 3 + s.points.lineWidth * 3, r * 3 + s.points.lineWidth * 3);
		} else {
	 ...
	 */
	drawHit : function(n) {
		var octx = this.octx, s = n.series, xa = n.xaxis, ya = n.yaxis;
	 ...
	 */
	hit : function(mouse) {
		var series = this.series, options = this.options, prevHit = this.prevHit, plotOffset = this.plotOffset, octx = this.octx, data, xsens, ysens, x, y, xa, ya, mx, my,
		/**
		...
		for( i = 0; i < series.length; i++) {
			...
			for(var j = 0, xpow, ypow; j < data.length; j++) {
				...
				if(y === null || xa.min > x || xa.max < x || ya.min > y || ya.max < y)
					continue;

				var xdiff = Math.abs(x - mx), ydiff = Math.abs(y - my);
		...
		if(n.series && (n.mouse && n.mouse.track && !prevHit || (prevHit /*&& (n.x != prevHit.x || n.y != prevHit.y)*/))) {
			var mt = this.mouseTrack || this.el.select(".flotr-mouse-value")[0], pos = '', s = n.series, p = n.mouse.position, m = n.mouse.margin, elStyle = 'opacity:0.7;background-color:#000;color:#fff;display:none;position:absolute;padding:2px 8px;-moz-border-radius:4px;border-radius:4px;white-space:nowrap;';
...
Flotr.Date = {
	...
	formatter : function(v, axis) {
		...
		var span = axis.max - axis.min, t = axis.tickSize * Flotr.Date.timeUnits[axis.tickUnit];

		return Flotr.Date.format(d, Flotr.Date.getFormat(t, span));
	},
	generator : function(axis) {
		var ticks = [], d = new Date(axis.min), tu = Flotr.Date.timeUnits;
		...
		do {
			...
			ticks.push({
				v : v,
				label : Flotr.Date.formatter(v, axis)
			});
...
Flotr.addType('lines', {
	...
	plot : function(series, offset) {
		var ctx = this.ctx, xa = series.xaxis, ya = series.yaxis, data = series.data;

		if(data.length < 2)
			return;

		var prevx = xa.d2p(data[0][0]), prevy = ya.d2p(data[0][1]) + offset;
	...
	plotArea : function(series, offset) {
		...
		var top, lastX = 0, ctx = this.ctx, xa = series.xaxis, ya = series.yaxis, bottom = Math.min(Math.max(0, ya.min), ya.max), first = true;
...
Flotr.addType('bars', {
	...
	draw : function(series) {
		...
		if(series.bars.fill) {
			...
			ctx.fillStyle = this.processColor(color, {
				opacity : series.bars.fillOpacity
			});
		}
	...
	plot : function(series, barWidth, offset, fill) {
		...
		var xa = series.xaxis, ya = series.yaxis, ctx = this.ctx;

		for(var i = 0; i < data.length; i++) {
			var x = data[i][0], y = data[i][1], drawLeft = true, drawTop = true, drawRight = true;
	...
	plotShadows : function(series, barWidth, offset) {
		...
		var xa = series.xaxis, ya = series.yaxis, ctx = this.ctx, sw = this.options.shadowSize;
	...
	extendXRange : function(axis) {
		if(axis.options.max == null) {
			var newmin = axis.min, newmax = axis.max, i, s, b, stackedSums = [], lastSerie = null;
	...
	extendYRange : function(axis) {
		if(axis.options.max == null) {
			var newmax = axis.max, i, s, b, stackedSums = {}, lastSerie = null;
...
Flotr.addType('pie', {
	...
	draw : function(series) {
		if(this.options.pie.drawn)
			return;
		var ctx = this.ctx, options = this.options, lw = series.pie.lineWidth, sw = series.shadowSize, data = series.data, plotOffset = this.plotOffset, radius = (Math.min(this.canvasWidth, this.canvasHeight) * series.pie.sizeRatio) / 2, html = [], vScale = 1, //Math.cos(series.pie.viewAngle);
		plotTickness = Math.sin(series.pie.viewAngle) * series.pie.spliceThickness / vScale, style = {
			size : options.fontSize * 1.2,
			color : options.grid.color,
			weight : 1.5
		}, center = {
		...
		sum = portions.pluck('value').pluck(1).inject(0, function(acc, n) {
			return acc + n;
		}), fraction = 0.0, angle = series.pie.startAngle, value = 0.0;

		var slices = portions.collect(function(slice) {
			angle += fraction;
			value = parseFloat(slice.value[1]);
			// @warning : won't support null values !!
			fraction = value / sum;
			return {
				name : slice.name,
				fraction : fraction,
		...
		if(sw > 0) {
			slices.each(function(slice) {
				...
				var bisection = (slice.startAngle + slice.endAngle) / 2, xOffset = center.x + Math.cos(bisection) * slice.options.explode + sw, yOffset = center.y + Math.sin(bisection) * slice.options.explode + sw;
		...
		slices.each(function(slice, index) {
			...
			var bisection = (slice.startAngle + slice.endAngle) / 2, color = slice.series.color, fillColor = slice.options.fillColor || color, xOffset = center.x + Math.cos(bisection) * slice.options.explode, yOffset = center.y + Math.sin(bisection) * slice.options.explode;

			this.pie.plotSlice(xOffset, yOffset, radius, slice.startAngle, slice.endAngle, false, vScale);

			if(series.pie.fill) {
				ctx.fillStyle = this.processColor(fillColor, {
					opacity : series.pie.fillOpacity
				});
				ctx.fill();
			}
			ctx.lineWidth = lw;
			...
			var label = options.pie.labelFormatter(slice), textAlignRight = (Math.cos(bisection) < 0), distX = xOffset + Math.cos(bisection) * (series.pie.explode + radius), distY = yOffset + Math.sin(bisection) * (series.pie.explode + radius);
...
Flotr.addType('candles', {
	...
	plot : function(series, offset) {
		...
		for(var i = 0; i < data.length; i++) {
			...
			var left = x - series.candles.candleWidth / 2, right = x + series.candles.candleWidth / 2, bottom = Math.max(ya.min, low), top = Math.min(ya.max, high), bottom2 = Math.max(ya.min, Math.min(open, close)), top2 = Math.min(ya.max, Math.max(open, close));
			...
			if(series.candles.fill && !series.candles.barcharts) {
				ctx.fillStyle = this.processColor(color, {
					opacity : series.candles.fillOpacity
				});
	...
	plotShadows : function(series, offset) {
		...
		var xa = series.xaxis, ya = series.yaxis, sw = this.options.shadowSize;

		for(var i = 0; i < data.length; i++) {
			var d = data[i], x = d[0], open = d[1], high = d[2], low = d[3], close = d[4];

			var left = x - series.candles.candleWidth / 2, right = x + series.candles.candleWidth / 2, bottom = Math.max(ya.min, Math.min(open, close)), top = Math.min(ya.max, Math.max(open, close));
	...
	extendXRange : function(axis) {
		if(axis.options.max == null) {
			var newmin = axis.min, newmax = axis.max, i, c;
...
Flotr.addType('markers', {
	...
	draw : function(series) {
		series = series || this.series;
		var ctx = this.ctx, xa = series.xaxis, ya = series.yaxis, options = series.markers, data = series.data;
		...
		ctx.fillStyle = this.processColor(options.fillColor, {
			opacity : options.fillOpacity
		});

		for(var i = 0; i < data.length; ++i) {
			var x = data[i][0], xPos = xa.d2p(x), y = data[i][1], yPos = ya.d2p(y), label = options.labelFormatter({
				x : x,
				y : y,
				index : i,
				data : data
	...
	plot : function(x, y, label, options) {
		var ctx = this.ctx, dim = this.getTextDimensions(label, null, null), margin = 2, left = x, top = y;
		...
		ctx.drawText(label, left + margin, top + margin, {
			valign : 't',
			halign : 'l'
		});
	}
...
Flotr.addType('radar', {
	...
	draw : function(series) {
		var ctx = this.ctx, options = this.options;
		...
		ctx.strokeStyle = series.color;
		ctx.fillStyle = this.processColor(series.color, {
			opacity : series.radar.fillOpacity
		});
	...
	plot : function(series, offset) {
		var ctx = this.ctx, options = this.options, data = series.data, radius = Math.min(this.plotHeight, this.plotWidth) * options.radar.radiusRatio / 2, coeff = 2 * (Math.PI / data.length), angle = -Math.PI / 2;
		offset = offset || 0;

		ctx.beginPath();
		for(var i = 0; i < data.length; ++i) {
			var x = data[i][0], y = data[i][1], ratio = y / this.axes.y.max;
...
Flotr.addType('bubbles', {
	...
	plot : function(series, offset) {
		var ctx = this.ctx, options = this.options, data = series.data, radius = options.bubbles.baseRadius;
		offset = offset || 0;

		for(var i = 0; i < data.length; ++i) {
			var x = data[i][0], y = data[i][1], z = data[i][2];
...
Flotr.addPlugin('spreadsheet', {
	...
	constructDataGrid : function() {
		...
		var i, j, s = this.series, datagrid = this.loadDataGrid(), t = this.spreadsheet.datagrid = new Element('table', {
			style : 'height:100px'
		}).addClassName('flotr-datagrid'), colgroup = ['<colgroup><col />'];
		...
		for( j = 0; j < datagrid.length; ++j) {
			...
			for( i = 0; i < s.length + 1; ++i) {
				var tag = 'td', content = (datagrid[j][i] != null ? Math.round(datagrid[j][i] * 100000) / 100000 : '');
		...
		var toolbar = new Element('div').addClassName('flotr-datagrid-toolbar').insert(new Element('button', {
			type : 'button'
	...
	downloadCSV : function() {
		var i, csv = '', series = this.series, options = this.options, dg = this.loadDataGrid(), separator = encodeURIComponent(options.spreadsheet.csvFileSeparator);
		...
		for( i = 0; i < dg.length; ++i) {
			...
			csv += rowLabel + separator + numbers + "%0D%0A";
			// \t and \r\n
		}
		if(Prototype.Browser.IE) {
